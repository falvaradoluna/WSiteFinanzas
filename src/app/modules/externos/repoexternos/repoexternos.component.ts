...
export class RepoexternosComponent implements OnInit {
	...
	createExcel(): void {
		...
		if (this.form.value.selectCompania != 0) {
			switch (this.form.value.selectPeriodo.toString()) {
				case '0': {
					console.log( 'Seleccione un periodo' );
				break;
				};
				case '1': {
					if( this.form.value.selectAnio == 0 ){
						console.log( 'Seleccione el año' );
					}else{
						this.sendParamsExcel();
					}
				break;
				};
				case'2': {
					if( this.form.value.selectAnio == 0 || this.form.value.selectMes == 0 ){
						console.log( 'Seleccione los campos necesarios' );
					}else{
						this.sendParamsExcel();
					}
				break;
			...
			console.log( 'Seleccione una compañia' );
		}
	};

	sendParamsExcel(): void{
		console.log('Aqui');
		if( this.form.value.selectPeriodo == 1 ){
			this._serviceExternos.createExcel({ 
				idCompania: this.form.value.selectCompania,
				resporteId: this.form.value.selectPeriodo,
